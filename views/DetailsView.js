                              ...
                              <AntDesign name="closecircleo" size={20} color={"black"} />
                                              ...
                                              <Text style={styles.textTitle}>Penalty Amount</Text>
                                                        <TextInput
                                                            ...
                                                            defaultValue= {checkinInfo.penalty== '' ||  checkinInfo.penalty == null||  checkinInfo.penalty == undefined?'0':checkinInfo.penalty}
                                                            keyboardType='numeric'
                                                            autoCorrect={true}
                                                        />
     ...
     {checkinInfo.voucher_mode ===""?null:   <ListItem>
        ...
        <Text style={{  fontSize: 13,fontWeight: 'bold'}}>B A L A N C E: </Text>
          ...
          currencyFormat(parseFloat(Order&& Order.length ?Order.reduce((sum, i) => ( checkinInfo.temp_id == i.customer_id && i.method != "Paid"?
            sum += (i.quantity * i.price):
            sum += 0
          ), 0) : 0)+parseFloat(extension_calcu)+parseFloat(checkinInfo.penalty== '' ||  checkinInfo.penalty == null||  checkinInfo.penalty == undefined? 0: checkinInfo.penalty)+parseFloat(extra)+parseFloat(stay_total)-parseFloat(discount)-parseFloat(checkinInfo.payment)-parseFloat(Voucher))
        } </Text>
              </Right>
        </ListItem>
...
<Modal visible={visibleBalanceModal}
          ...
          currencyFormat(parseFloat(Order&& Order.length ?Order.reduce((sum, i) => ( checkinInfo.temp_id == i.customer_id && i.method != "Paid"?
            ...
            sum += 0
                                             ...
                                             {parseFloat(checkinInfo.payment)>= parseFloat(Order&& Order.length ?Order.reduce((sum, i) => ( checkinInfo.temp_id == i.customer_id && i.method != "Paid"?
            sum += (i.quantity * i.price):
            sum += 0
          ), 0) : 0)+parseFloat(extension_calcu)+parseFloat(checkinInfo.penalty== '' ||  checkinInfo.penalty == null||  checkinInfo.penalty == undefined? 0: checkinInfo.penalty)+parseFloat(extra)+parseFloat(stay_total)-parseFloat(discount)-parseFloat(checkinInfo.payment)-parseFloat(Voucher)?   <TouchableOpacity
