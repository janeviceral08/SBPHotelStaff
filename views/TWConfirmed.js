...
export function TWConfirmed({ navigation, route }) {
 ...
 const TodayReserve = Booking.filter(item =>item.status === 'Confirmed' && item.in_check < moment(time).add(7, 'days').unix())
...
{ cancelable: false }
  ...
  const Details = (project) => {
          ...
          navigation.navigate("TWConfirmedItem", {
  ...
  return (
    <View>
        ...
        <Text style={{marginTop: 20, fontWeight: 'bold', color: 'white'}}>This Week Reservations</Text>
       
       </Header>
<ScrollView style={{marginBottom: 2}}>
              {  TodayReserve.map((checkin, i) =>
