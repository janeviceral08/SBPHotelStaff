...
<Modal visible={visibleBalanceModal}
          ...
          currencyFormat(parseFloat(Order&& Order.length ?Order.reduce((sum, i) => ( checkinInfo.temp_id == i.customer_id && i.method != "Paid"?
            ...
            sum += (i.quantity * i.price):
            sum += 0 ), 0) : 0)+parseFloat(extension_calcu)+parseFloat(checkinInfo.penalty== '' ||  checkinInfo.penalty == null||  checkinInfo.penalty == undefined? 0: checkinInfo.penalty)+parseFloat(extra)+parseFloat(stay_total)-parseFloat(discount)-parseFloat(Voucher)-parseFloat(checkinInfo.payment)?
           <TouchableOpacity
